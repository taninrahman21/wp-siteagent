		...
		<div class="sa-container">
			...
			<div class="sa-top-status">
				...
				<div class="sa-status-center">
					...
					<button type="button" class="sa-copy-btn sa-copy-btn--inline" onclick="siteagent.copyText('sa-mcp-url-top')">
				...
				<div class="sa-dashboard-main">
					...
					<div class="sa-card" style="margin-bottom: 0; height: 100%;">
						<div class="sa-card-header" style="border-bottom: none; padding-bottom: 0;">
							<h2 style="font-size: 14px; font-weight: 600;"><?php esc_html_e( 'Recent audit log', 'wp-siteagent' ); ?></h2>
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=wp-siteagent-audit' ) ); ?>" class="sa-link" style="font-size: 12px; color: var(--sa-primary); text-decoration: none; font-weight: 500;"><?php esc_html_e( 'View all →', 'wp-siteagent' ); ?></a>
						</div>
								...
								<table class="sa-table" style="border-top: none;">
									<thead>
										<tr style="background: transparent;">
											<th style="background: transparent; border-bottom: 1px solid var(--sa-border); padding: 16px 24px; color: var(--sa-text-muted); font-size: 11px;"><?php esc_html_e( 'ABILITY', 'wp-siteagent' ); ?></th>
											<th style="background: transparent; border-bottom: 1px solid var(--sa-border); padding: 16px 24px; color: var(--sa-text-muted); font-size: 11px;"><?php esc_html_e( 'TOKEN', 'wp-siteagent' ); ?></th>
											<th style="background: transparent; border-bottom: 1px solid var(--sa-border); padding: 16px 24px; color: var(--sa-text-muted); font-size: 11px;"><?php esc_html_e( 'STATUS', 'wp-siteagent' ); ?></th>
											<th style="background: transparent; border-bottom: 1px solid var(--sa-border); padding: 16px 24px; color: var(--sa-text-muted); font-size: 11px;"><?php esc_html_e( 'TIME', 'wp-siteagent' ); ?></th>
										</tr>
									</thead>
									<tbody>
										<?php 
										...
										?>
										<tr>
											<td style="font-family: var(--sa-font-mono); font-size: 13px;"><?php echo esc_html( $log['ability_name'] ); ?></td>
											<td style="color: var(--sa-text-secondary); font-size: 13px;"><?php echo esc_html( $token_name ); ?></td>
											<td>
												<span class="sa-badge sa-badge--<?php echo esc_attr( $log['result_status'] ); ?>">
													<?php echo esc_html( $log['result_status'] ); ?>
												</span>
											</td>
											<td style="color: var(--sa-text-muted); font-size: 13px;"><?php echo esc_html( human_time_diff( strtotime( $log['executed_at'] ) ) ); ?> <?php esc_html_e( 'ago', 'wp-siteagent' ); ?></td>
										</tr>
										<?php endforeach; ?>
									</tbody>
								</table>
				...
				<div class="sa-dashboard-sidebar">
					...
					<div class="sa-card" style="margin-bottom: 24px;">
						<div class="sa-card-header" style="border-bottom: none; padding-bottom: 8px;">
							<h2 style="font-size: 14px; font-weight: 600;"><?php esc_html_e( 'MCP config', 'wp-siteagent' ); ?></h2>
						</div>
						<div class="sa-card-body" style="padding-top: 8px;">
							...
							?>
							<div class="sa-mcp-block" id="sa-mcp-config-block">
								<button type="button" class="sa-copy-btn" onclick="siteagent.copyText('sa-mcp-config-json')"><?php esc_html_e( 'Copy', 'wp-siteagent' ); ?></button>
								<pre><code id="sa-mcp-config-json"><?php echo esc_html( $config_json ); ?></code></pre>
							</div>
						</div>
					</div>
					...
					<div class="sa-card" style="margin-bottom: 0;">
						<div class="sa-card-header" style="border-bottom: 1px solid var(--sa-border); padding-bottom: 16px;">
							<h2 style="font-size: 14px; font-weight: 600;"><?php esc_html_e( 'Module status', 'wp-siteagent' ); ?></h2>
						</div>
							...
							<div class="sa-modules-grid">
								...
								<div class="sa-module-card">
									<div class="sa-module-header">
										<span class="sa-module-title"><?php echo esc_html( $def['label'] ); ?></span>
										<span class="sa-module-status-dot <?php echo esc_attr( $status_class ); ?>"></span>
									</div>
									<span class="sa-module-count"><?php printf( esc_html__( '%d abilities', 'wp-siteagent' ), $ability_count ); ?></span>
								</div>
								<?php endforeach; ?>
							</div>
						</div>
