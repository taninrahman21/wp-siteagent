			...
			<div class="sa-page-header" style="display: flex; justify-content: space-between; align-items: center;">
				<div>
					...
					<p class="sa-page-desc">
						<?php esc_html_e('Manage access tokens for your MCP clients. Revoked tokens are immediately invalidated.', 'wp-siteagent'); ?>
					</p>
				</div>
					...
					<?php if (empty($siteagent_tokens)): ?>
						<div class="sa-empty-state" style="padding: 60px 24px; text-align: center;">
							...
							<p style="font-size: 15px; color: var(--sa-text-secondary);">
								<?php esc_html_e('No tokens yet. Generate your first token to connect an MCP client.', 'wp-siteagent'); ?>
							</p>
						</div>
					<?php else: ?>
								...
								<tbody>
									<?php foreach ($siteagent_tokens as $siteagent_token):
										...
										if (empty($siteagent_token['abilities'])) {
											$siteagent_abilities_text = __('All abilities', 'wp-siteagent');
										} else {
										...
										<tr>
											...
											<td class="sa-td--time">
												<?php echo $siteagent_token['expires_at'] ? esc_html(wp_date(get_option('date_format'), strtotime($siteagent_token['expires_at']))) : '<em>' . esc_html__('Never', 'wp-siteagent') . '</em>'; ?>
											</td>
											<td class="sa-td--time">
												<?php echo $siteagent_token['last_used'] ? esc_html(human_time_diff(strtotime($siteagent_token['last_used'])) . ' ' . __('ago', 'wp-siteagent')) : '<em>' . esc_html__('Never', 'wp-siteagent') . '</em>'; ?>
											</td>
											...
											<td><span
													class="sa-badge sa-badge--<?php echo esc_attr($siteagent_status); ?>"><?php echo esc_html($siteagent_status); ?></span>
											</td>
		...
		<div class="sa-modal">
			...
			<div class="sa-modal-body">
				<form id="sa-generate-token-form">
					<div class="sa-form-group">
						<label for="sa-token-label"><?php esc_html_e('Label', 'wp-siteagent'); ?> <span
								class="sa-required">*</span></label>
						<input type="text" id="sa-token-label" name="label" class="sa-input"
							placeholder="<?php esc_attr_e('e.g. Claude Desktop - Home', 'wp-siteagent'); ?>"
							required />
						<p class="sa-hint">
							<?php esc_html_e('A descriptive name to help you identify this token later.', 'wp-siteagent'); ?>
						</p>
					</div>
					<div class="sa-form-group">
					...
					<div class="sa-form-group">
						...
						<p class="sa-hint">
							<?php esc_html_e('Leave all unchecked to allow all abilities.', 'wp-siteagent'); ?></p>
						...
						<div
							...
							<div id="sa-claude-panel">
								<h4
									...
									<?php esc_html_e('Connect Claude Desktop', 'wp-siteagent'); ?></h4>
								...
								<p
									...
									<?php esc_html_e('Run Step 1 first and wait for it to complete before running Step 2.', 'wp-siteagent'); ?>
								</p>
							...
							<div id="sa-cursor-panel" style="display:none;">
								<h4
									style="margin: 0 0 12px; font-size: 14px; font-weight: 700; color: var(--sa-primary);">
									<?php esc_html_e('Connect Cursor', 'wp-siteagent'); ?></h4>

								<div class="sa-form-group" style="margin-bottom: 12px;">
									<label
										style="font-size: 11px; font-weight: 700; margin-bottom: 4px; display: block; color: var(--sa-text-secondary);"><?php esc_html_e('MCP Server URL', 'wp-siteagent'); ?></label>
									<div class="sa-token-value-wrap">
								...
								<div class="sa-form-group" style="margin-bottom: 12px;">
									<label
										style="font-size: 11px; font-weight: 700; margin-bottom: 4px; display: block; color: var(--sa-text-secondary);"><?php esc_html_e('Type', 'wp-siteagent'); ?></label>
									<div class="sa-token-value-wrap">
								...
								<div class="sa-form-group" style="margin-bottom: 12px;">
									<label
										style="font-size: 11px; font-weight: 700; margin-bottom: 4px; display: block; color: var(--sa-text-secondary);"><?php esc_html_e('Authorization Header', 'wp-siteagent'); ?></label>
									<div class="sa-token-value-wrap">
			...
			<div class="sa-modal-footer">
				<button type="button" class="sa-btn sa-btn--ghost"
					onclick="siteagentTokens.closeModal()"><?php esc_html_e('Cancel', 'wp-siteagent'); ?></button>
