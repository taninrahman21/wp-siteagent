	...
	<div class="sa-main-content">
		<div class="sa-container">
			<div class="sa-page-header" style="margin-bottom: 32px; border-bottom: 1px solid var(--sa-border); padding-bottom: 24px;">
				<h2 style="margin: 0; font-size: 24px; font-weight: 700; color: var(--sa-secondary);"><?php esc_html_e( 'Settings', 'wp-siteagent' ); ?></h2>
				<p style="margin: 8px 0 0; font-size: 14px; color: var(--sa-text-muted);"><?php esc_html_e( 'Configure the core behavior of the MCP server and available tool modules.', 'wp-siteagent' ); ?></p>
			</div>
			...
			<div class="sa-dashboard-body">
				...
				<div class="sa-dashboard-main">
					...
					<div class="sa-card">
						<div class="sa-card-header">
							<h3><?php esc_html_e( 'General Configuration', 'wp-siteagent' ); ?></h3>
						</div>
						<div class="sa-card-body">
							<div class="sa-setting-item">
								<div class="sa-setting-info">
									<span class="sa-setting-title"><?php esc_html_e( 'Enable MCP Server', 'wp-siteagent' ); ?></span>
									<span class="sa-setting-desc"><?php esc_html_e( 'Activate the Model Context Protocol server for this site. Disabling this will shut down the endpoint for all agents.', 'wp-siteagent' ); ?></span>
								</div>
							...
							<div class="sa-setting-item">
								<div class="sa-setting-info" style="padding-top: 8px;">
									<span class="sa-setting-title"><?php esc_html_e( 'Agent Display Name', 'wp-siteagent' ); ?></span>
									<span class="sa-setting-desc"><?php esc_html_e( 'The localized name that AI clients (like Claude or Cursor) will see during initialization.', 'wp-siteagent' ); ?></span>
									<input type="text" id="siteagent_display_name" name="siteagent_display_name"
										...
										class="sa-input" style="margin-top: 12px; max-width: 400px;" placeholder="e.g. My Website Agent"
					...
					<div class="sa-card" style="margin-top: 32px;">
						<div class="sa-card-header">
							<h3><?php esc_html_e( 'Active Modules', 'wp-siteagent' ); ?></h3>
						</div>
						<div class="sa-card-body">
				...
				<div class="sa-dashboard-sidebar">
					...
					<div class="sa-card">
						...
						<div class="sa-card-body">
							<div class="sa-form-group">
								<span class="sa-setting-title"><?php esc_html_e( 'Hourly Limit', 'wp-siteagent' ); ?></span>
								...
								<p class="sa-hint"><?php esc_html_e( 'Calls allowed per hour per token.', 'wp-siteagent' ); ?></p>
							</div>
							<div class="sa-form-group" style="margin-top: 20px;">
								<span class="sa-setting-title"><?php esc_html_e( 'Daily Limit', 'wp-siteagent' ); ?></span>
								...
								<p class="sa-hint"><?php esc_html_e( 'Calls allowed per day per token.', 'wp-siteagent' ); ?></p>
							</div>
						</div>
					</div>
					...
					<div class="sa-card">
						<div class="sa-card-header">
							<h3><?php esc_html_e( 'System Cache', 'wp-siteagent' ); ?></h3>
						</div>
						<div class="sa-card-body">
							<div class="sa-form-group">
								<span class="sa-setting-title"><?php esc_html_e( 'Cache TTL', 'wp-siteagent' ); ?></span>
								<div style="display: flex; gap: 8px; align-items: center; margin-top: 8px;">
									...
									<button type="button" class="sa-btn sa-btn--ghost sa-btn--sm" onclick="siteagent.clearCache()"><?php esc_html_e( 'Flush', 'wp-siteagent' ); ?></button>
								</div>
								<p class="sa-hint"><?php esc_html_e( 'Results TTL (seconds).', 'wp-siteagent' ); ?></p>
							</div>
						</div>
					</div>
					...
					<div class="sa-card">
						<div class="sa-card-header">
							<h3><?php esc_html_e( 'Audit Logging', 'wp-siteagent' ); ?></h3>
						</div>
						<div class="sa-card-body">
							<div class="sa-form-group">
								<span class="sa-setting-title"><?php esc_html_e( 'Retention', 'wp-siteagent' ); ?></span>
								<div style="display: flex; align-items: center; gap: 8px; margin-top: 8px;">
									...
									<span style="font-size: 13px; color: var(--sa-text-muted);"><?php esc_html_e( 'days', 'wp-siteagent' ); ?></span>
								</div>
							</div>
							<div class="sa-form-group" style="margin-top: 20px;">
								<span class="sa-setting-title"><?php esc_html_e( 'Detail Level', 'wp-siteagent' ); ?></span>
								<select name="siteagent_log_level" class="sa-select" style="margin-top: 8px;" onchange="siteagent.saveOption('siteagent_log_level', this.value)">
									<option value="all" <?php selected( get_option( 'siteagent_log_level', 'all' ), 'all' ); ?>><?php esc_html_e( 'Detailed (All calls)', 'wp-siteagent' ); ?></option>
									<option value="errors-only" <?php selected( get_option( 'siteagent_log_level' ), 'errors-only' ); ?>><?php esc_html_e( 'Errors Only', 'wp-siteagent' ); ?></option>
									<option value="none" <?php selected( get_option( 'siteagent_log_level' ), 'none' ); ?>><?php esc_html_e( 'Off', 'wp-siteagent' ); ?></option>
								</select>
							</div>
						</div>
					</div>
					...
					<div class="sa-card sa-danger-card">
						<div class="sa-card-header sa-danger-header">
							<h3 class="sa-danger-title"><?php esc_html_e( 'Maintenance', 'wp-siteagent' ); ?></h3>
						</div>
						<div class="sa-card-body" style="padding: 20px;">
							<div style="margin-bottom: 20px;">
								<span class="sa-setting-title" style="font-size: 13px;"><?php esc_html_e( 'Full Reset', 'wp-siteagent' ); ?></span>
								<button type="button" class="sa-btn sa-btn--sm sa-btn--ghost" style="width: 100%; margin-top: 8px; color: var(--sa-danger); border-color: rgba(214,54,56,0.1);" onclick="siteagent.dangerAction('reset_all', '<?php echo esc_js( wp_create_nonce( 'siteagent_admin' ) ); ?>')">
									<?php esc_html_e( 'Reset All Data', 'wp-siteagent' ); ?>
								</button>
							</div>
							<div style="display: flex; justify-content: space-between; align-items: center;">
								<span class="sa-setting-title" style="margin-bottom: 0; font-size: 13px;"><?php esc_html_e( 'Auto-Cleanup', 'wp-siteagent' ); ?></span>
								<label class="sa-switch">
			...
			<div class="sa-settings-footer" style="margin-top:48px; padding-top: 32px; border-top: 1px solid var(--sa-border); display:flex; justify-content:flex-end;">
				<?php submit_button( __( 'Save All Settings', 'wp-siteagent' ), 'sa-btn sa-btn--primary', 'submit', false ); ?>
			</div>
			</form>
		</div>
